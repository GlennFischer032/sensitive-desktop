...
def validate_content_length(
    max_length: int = RequestValidationConfig.MAX_CONTENT_LENGTH,
):
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            if request.method in ["POST", "PUT", "PATCH"]:
                ...
                if not content_type:
                    return (
                        jsonify({"error": "Content-Type header is required"}),
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            ...
            for (
                header,
                value,
            ) in RequestValidationConfig.REQUIRED_SECURITY_HEADERS.items():
