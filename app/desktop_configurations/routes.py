...
from . import desktop_config_bp

logger = logging.getLogger(__name__)
desktop_configs_client = DesktopConfigurationsClient()


@desktop_config_bp.route("/list")
@login_required
def list_configurations() -> str:
    ...
    try:
        ...
        return render_template("desktop_configurations.html", configurations=configs)
    except APIError as e:
        logger.error(f"Error listing configurations: {str(e)}")
        flash(f"Error listing configurations: {str(e)}", "error")
        return render_template("desktop_configurations.html", configurations=[])


@desktop_config_bp.route("/create", methods=["GET", "POST"])
@admin_required
def create_configuration() -> Response | str:
    ...
    if request.method == "POST":
        try:
            ...
            if request.is_json:
                ...
                return redirect(url_for("desktop_configurations.list_configurations"))
    ...
    return render_template(
        "desktop_configuration_form.html", configuration=default_config, users=all_users
    )


@desktop_config_bp.route("/edit/<int:config_id>", methods=["GET", "POST"])
@admin_required
def edit_configuration(config_id: int) -> Response | str:
    ...
    try:
        ...
        return render_template(
            "desktop_configuration_form.html", configuration=config, users=all_users
        )
    except APIError as e:
...
@desktop_config_bp.route("/delete/<int:config_id>", methods=["POST"])
@admin_required
