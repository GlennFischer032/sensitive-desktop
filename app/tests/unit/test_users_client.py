...
@patch("app.clients.base.BaseClient.post")
def test_update_user_single_field(mock_post):
    """
    GIVEN a UsersClient
    WHEN update_user() is called with a single field
    THEN check it calls the API correctly
    """
    # Set up mock
    mock_response = (
        {"id": "user1", "username": "johndoe", "organization": "New Org"},
        200,
    )
    mock_post.return_value = mock_response
    ...
    # Call method
    client = UsersClient()
    result = client.update_user(username="johndoe", organization="New Org")
...
@patch("app.clients.base.BaseClient.post")
def test_update_user_multiple_fields(mock_post):
    """
    GIVEN a UsersClient
    WHEN update_user() is called with multiple fields
    THEN check it calls the API correctly
    """
    # Set up mock
    mock_response = (
...
@patch("app.clients.base.BaseClient.post")
def test_update_user_no_fields(mock_post):
    """
    GIVEN a UsersClient
    WHEN update_user() is called with no fields
    THEN check it raises an APIError
    """
    # Call method and verify exception
    client = UsersClient()
    with pytest.raises(APIError) as exc_info:
        client.update_user(username="johndoe")
...
@patch("app.clients.base.BaseClient.post")
def test_update_user_error(mock_post):
    """
    GIVEN a UsersClient
    WHEN update_user() is called and the API returns an error
    THEN check it raises an APIError
    """
    # Set up mock
    mock_post.side_effect = APIError("User not found", 404)
    ...
    client = UsersClient()
    with pytest.raises(APIError):
        client.update_user(username="nonexistent", organization="New Org")
