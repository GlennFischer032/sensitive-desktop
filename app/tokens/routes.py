    ...
    try:
        ...
        for token in tokens:
            if "created_at" in token and token["created_at"]:
                token["created_at"] = parse_date_safely(token["created_at"])
            if "expires_at" in token and token["expires_at"]:
                token["expires_at"] = parse_date_safely(token["expires_at"])
            if "last_used" in token and token["last_used"]:
                token["last_used"] = parse_date_safely(token["last_used"])
            if "revoked_at" in token and token["revoked_at"]:
                token["revoked_at"] = parse_date_safely(token["revoked_at"])

        # Add current datetime for comparison in template
        now = datetime.utcnow()

        # Get the new token from query parameters if present
        new_token = None
