...
def get_pvc(pvc_id):
    ...
    try:
        storage_client = client_factory.get_storage_client()
        pvc = storage_client.get_storage(pvc_id, token=session["token"])
...
def get_pvc_access(pvc_id):
    ...
    try:
        storage_client = client_factory.get_storage_client()
        data = storage_client.get_pvc_access(pvc_id, token=session["token"])
...
def update_pvc_access(pvc_id):
    ...
    try:
        ...
        storage_client = client_factory.get_storage_client()
        result = storage_client.update_pvc_access(pvc_id, is_public, allowed_users, token=session["token"])
...
def get_pvc_connections(pvc_id):
    ...
    try:
        storage_client = client_factory.get_storage_client()
        connections = storage_client.get_pvc_connections(pvc_id, token=session["token"])
